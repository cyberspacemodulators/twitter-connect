...
public class IFLookAndFeel {
    ...
    public IFLookAndFeel(final char type) {
        defaultGraphicsState = new IFPGraphicsState();
    }

    public IFLookAndFeel(final PApplet parent, final char type) {
        defaultGraphicsState = new IFPGraphicsState();

        if (type == DEFAULT) {
            // Play nicely with other people's draw methods. They
            // may have changed the color mode.
            final IFPGraphicsState temp = new IFPGraphicsState(parent);
			...
			*/
            ...
            final Path path = Paths.get("src/data/FrutigerLight-12.vlw");
            final File file = new File(path.toUri());
            final PFont tempFont = parent.loadFont(file.toString());
